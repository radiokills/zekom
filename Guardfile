...
guard 'rspec', :cli => "--color --format nested --drb --fail-fast", :version => 2 do
  ...
  watch(%r{^app/controllers/zekom/(.+)\.rb$})     { "spec/controllers" }
  watch(%r{^spec/controllers/(.+)\.rb$})     { "spec/controllers" }

  # Rails example
  watch(%r{^spec/dummy/app/(.+)\.rb$})                           { |m| "spec/#{m[1]}_spec.rb" }
  watch(%r{^spec/dummy/app/(.*)(\.erb|\.haml)$})                 {"spec/views"}
  ...
  #watch('spec/dummy/app/controllers/application_controller.rb') 
